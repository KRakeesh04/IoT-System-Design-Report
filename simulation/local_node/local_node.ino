...
static void requestRefill() {
  ...
  if (response.length() == 0) { refillActive = false; setValveOpen(false); return; }
  if (response.indexOf("APPROVE") >= 0) { refillActive = true; setValveOpen(true); }
  else { refillActive = false; setValveOpen(false); }
}

static void pollControl() {
  if (!refillActive) return;
  if (millis() - lastControlPoll < 250) return;
  lastControlPoll = millis();
  String response = httpGet("/control");
  if (response.length() == 0) return;
  if (response.indexOf("OPEN") >= 0) setValveOpen(true);
  else if (response.indexOf("CLOSE") >= 0) { setValveOpen(false); refillActive = false; }
  else if (response.indexOf("FAULT") >= 0 || response.indexOf("DENY") >= 0) { setValveOpen(false); refillActive = false; }
}
...
void loop() {
  if (WiFi.status() != WL_CONNECTED) connectToCentralWiFi();
