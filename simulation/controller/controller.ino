...
// Simulation override: if >=0, used instead of sensor
...
static float getDistanceCM() {
  if (simulatedDistanceCM >= 0.0) return simulatedDistanceCM;
...
static void handleRequest() {
  ...
  if (refillSessionActive) { server.send(409, "text/plain", "BUSY"); return; }
  if (waterLevel < LOW_WATER_THRESHOLD_CM) {
    displayMessage("FAULT", "LOW WATER");
    faultLatched = true; faultState();
...
static void handleControl() {
  if (faultLatched) { server.send(200, "text/plain", "FAULT"); return; }
  if (!refillSessionActive) { server.send(200, "text/plain", "CLOSE"); return; }
  if (flowPulseCount >= TARGET_PULSES) { stopRefillSession(); displayMessage("REFILL DONE", "Target Reached"); server.send(200, "text/plain", "CLOSE"); return; }
  if (millis() - lastFlowPulseTime > FLOW_TIMEOUT) {
    displayMessage("FAULT", "NO FLOW"); faultLatched = true; refillSessionActive = false; faultState(); server.send(200, "text/plain", "FAULT"); return;
  }
...
static void handleSerialCommands() {
  ...
  if (cmd == "R" || cmd == "r") { simulatedDistanceCM = -1.0; Serial.println("[SIM] distance override disabled"); }
  else if (cmd.startsWith("D") || cmd.startsWith("d")) { simulatedDistanceCM = cmd.substring(1).toFloat(); Serial.print("[SIM] distance set to "); Serial.println(simulatedDistanceCM); }
  else if (cmd == "ANODE") { rgbCommonAnode = true; Serial.println("[CFG] RGB common anode"); }
  else if (cmd == "CATHODE") { rgbCommonAnode = false; Serial.println("[CFG] RGB common cathode"); }
}

static void monitorWaterLevel() {
  float waterLevel = getWaterLevelCM();
  if (waterLevel < LOW_WATER_THRESHOLD_CM) {
    if (!lowWaterAlertActive) { lowWaterAlertActive = true; faultLatched = true; refillSessionActive = false; displayMessage("FAULT", "LOW WATER"); faultState(); }
    return;
  }
  if (lowWaterAlertActive) lowWaterAlertActive = false;
  if (!refillSessionActive && !faultLatched) { setRGBColor(false, true, false); displayMessage("System Started", "Ready"); }
}

static void manageRefill() {
  if (!refillSessionActive) return;
  if (flowPulseCount >= TARGET_PULSES) return;
  if (flowPulseCount == 0) setRGBColor(true, true, false); else setRGBColor(false, false, true);
  display.clearDisplay(); display.setCursor(0,0); display.println("REFILL ACTIVE"); display.print("Pulses: "); display.print(flowPulseCount); display.print("/"); display.println(TARGET_PULSES); display.display();
}
